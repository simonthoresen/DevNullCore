...
- Asset/script discovery is multi-root (`internal/engine/sources.go`): **Create > Shared > Common** with name shadowing.
...
- **Respect source precedence:** Create/Shared/Common order must stay consistent for listing and resolving games/plugins/shaders.
...
- **Data-dir behavior in dev:** when running from source, use `--data-dir dist/Common` for server/client commands to match packaged runtime layout.
