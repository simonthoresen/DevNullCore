...
Per-player (or per-console) JavaScript extensions in `dist/Core/Plugins/`. Loaded with `/plugin load <name|url>`. Each player/console maintains their own plugin list -- plugins are not shared.
...
Per-player (or per-console) post-processing scripts in `dist/Core/Shaders/`. Loaded with `/shader load <name|url>`. Each player/console maintains their own ordered shader list. Shaders run in sequence on the fully-rendered `ImageBuffer` **after** the screen is composed but **before** overlays (menus, dialogs) and `ToString()`.
...
**Charmap format:** Each charmap lives in `dist/Core/Charmaps/<name>/` with a `charmap.json` and an atlas PNG:
```json
{
  "name": "pacman",
  "version": 1,
...
| Package | Role |
|---------|------|
| `Core/charmap.go` | CharMapDef/CharMapEntry types, PUA constants, JSON loader |
| `Core/osc.go` | OSC escape sequence encoding for charmap/atlas/viewport/frame, bandwidth estimator |
