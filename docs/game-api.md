...
State separation: `Unload()` returns **persistent state** (high scores, unlocks) written to `dist/Common/state/<game>.json` and loaded on every fresh load and resume. `Suspend()` returns **session state** (board, current score) written only to the suspend save file and passed to `Resume()`. There is no `State()`/`SetState()` on `domain.Game`; `ScriptRuntime.State()` exists only for OSC push to local renderers.

## Games (JS)

Games live in `dist/Common/Games/` as either single `.js` files or folders containing `main.js` (for multi-file games using `include()`). Loaded at runtime via `/game load <name>`. A HTTPS URL can be given instead of a name -- downloads land in `~/DevNull/Shared/Games/` (`.js` files are cached in `Shared/Games/.cache/`, `.zip` files extracted into `Shared/Games/<name>/`). GitHub blob URLs are converted to raw automatically.
