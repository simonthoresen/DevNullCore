...
func (p *Program) Run() (returnModel Model, returnErr error) {
	...
	if p.renderer == nil {
		if p.disableRenderer {
			...
			// mapNl=true when there is no real TTY input (e.g. SSH/Wish sessions).
			// In non-TTY mode the output stream goes through gossh's PtyWriter
			// which converts bare \n → \r\n. If mapNl=false (the old Windows
			// hard-code), the renderer tracks \n as "col stays" while the
			// terminal resets to col 0 after \r\n, causing catastrophic cursor
			// drift in delta renders. Setting mapNl based only on ttyInput (not
			// GOOS) makes SSH-served bubbletea apps render correctly on Windows.
			// See: https://github.com/charmbracelet/bubbletea/issues/1572
